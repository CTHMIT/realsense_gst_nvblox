...
from gst_realsense_launch.startup.rs_detect import DeviceInfo, Mode, RealSenseDetector

realsense_camera = RealSenseDetector()
...
def test_find_best_mode_exact_match(sample_device_info):
    ...
    target_size = (640, 480)
    mode = realsense_camera.find_best_mode(sample_device_info, target_size, "color")
...
def test_find_best_mode_no_match(sample_device_info):
    ...
    target_size = (640, 480)
    mode = realsense_camera.find_best_mode(sample_device_info, target_size, "non_existent_type")
    assert mode is None
